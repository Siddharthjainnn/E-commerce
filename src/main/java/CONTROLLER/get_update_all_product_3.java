...
@WebServlet("/get_update_all_product_3")
public class get_update_all_product_3 extends HttpServlet {
	...
	if(pp!=null)
	{
		RequestDispatcher rd=request.getRequestDispatcher("update_page_all_product_deal_of_the_week.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 */admin_model model=new admin_model();
		int i=model.add_update_product_deal_of(a,b,c,d,e,f,g,h);
		...
		if(i!=0)
		{
			...
			request.setAttribute("msg", "product Added to deal of the day");
			request.setAttribute("list", list);
			rd.forward(request, response);
			
		}else {
			RequestDispatcher rd=request.getRequestDispatcher("Offer_page.jsp");
			request.setAttribute("msg", "product not added to deal of day  ");
			request.setAttribute("list", list);
			rd.forward(request, response);
