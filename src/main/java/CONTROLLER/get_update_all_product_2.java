...
/**
 * Servlet implementation class get_update_all_product_2
 */
@MultipartConfig(maxFileSize = 160300000)
@WebServlet("/get_update_all_product_2")
public class get_update_all_product_2 extends HttpServlet {
	...
	if(pp!=null)
	{
		RequestDispatcher rd=request.getRequestDispatcher("update_page_all_product_commbo.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 */admin_model model=new admin_model();
		int i=model.add_update_product_combo(a,b,c,d,e,f,g,h);
		...
		if(i!=0)
		{
			...
			request.setAttribute("msg", "product Added to comboo succesffuly");
			request.setAttribute("list", list);
			rd.forward(request, response);
			
		}else {
			RequestDispatcher rd=request.getRequestDispatcher("Offer_page.jsp");
			request.setAttribute("msg", "product not added to comboo  ");
			request.setAttribute("list", list);
			rd.forward(request, response);
