...
@WebServlet("/get_update_all_product_1")
public class get_update_all_product_1 extends HttpServlet {
	...
	if(pp!=null)
	{
		RequestDispatcher rd=request.getRequestDispatcher("update_page_all_product_offer.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 */admin_model model=new admin_model();
		int i=model.add_update_product_offer(a,b,c,d,e,f,g,h);
		...
		if(i!=0)
		{
			...
			request.setAttribute("msg", "product Added to Offer succesffuly");
			request.setAttribute("list", list);
			rd.forward(request, response);
			
		}else {
			RequestDispatcher rd=request.getRequestDispatcher("Offer_page.jsp");
			request.setAttribute("msg", "product not added to offer  ");
			request.setAttribute("list", list);
			rd.forward(request, response);
