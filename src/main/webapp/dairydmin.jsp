...
<%} %> 
	...
	<thead class="thead-dark">
		...
		<tr>
			...
			<td><%=mm.getPrice()%></td>
			<td><img alt="image not found" src="./get_image_admin_dairy?pro_id=<%=mm.getPro_id()%>"  height="200px" ,width="200px"></td>
		   <%--  <td><img alt="image not found" src="./get_image_1?user_id=<%=mm.getUser_id()%>"  height="200px" ,width="200px"></td>
		    ...
		    <td><a href="./get_delete_dairy?user_id=<%=mm.getPro_id()%>" class="btn btn-danger" onclick=" return alert('are you sure you whant to delete')">DELETE</a></td>
		    <td><a href="./get_update_dairy?user_id=<%=mm.getPro_id()%>" class="btn btn-primary">UPDATE</a></td> 
	</tr>
		<%
		}
		%>
